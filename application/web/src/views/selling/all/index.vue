      ...
      <el-col v-for="(val,index) in sellingList" :key="index" :span="6" :offset="1">
        <el-card class="all-card">
          <div slot="header" class="clearfix">
            ...
            <el-button v-if="roles[0] !== 'admin'&&(val.seller===accountId||val.buyer===accountId)&&val.sellingStatus!=='Finished'&&val.sellingStatus!=='Expired'&&val.sellingStatus!=='Cancelled'" style="float: right; padding: 3px 0" type="text" @click="updateSelling(val,'cancelled')">Cancel</el-button>
            <el-button v-if="roles[0] !== 'admin'&&val.seller===accountId&&val.sellingStatus==='In Delivery'" style="float: right; padding: 3px 8px" type="text" @click="updateSelling(val,'done')">Confirm Payment</el-button>
          ...
          <div class="item">
            ...
            <span>{{ val.salePeriod }} day(s)</span>
          </div>
          <div class="item">
          ...
          <div class="item">
            ...
            <span v-if="val.buyer===''">Waiting for buyer</span>
            <span>{{ val.buyer }}</span>
          </div>
        </el-card>
      </el-col>
...
export default {
  ...
  methods: {
    createSellingByBuy(item) {
      this.$confirm('Do you want to buy now?', 'Reminder', {
